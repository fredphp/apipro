...
func (m *ChatRoomMessageModel) Insert(ctx context.Context, msg *ChatRoomMessage) error {
        ...
        _, err := m.db.ExecContext(ctx, `
                INSERT INTO `+Tbl("chat_room_message")+` (chat_room_message_id, send_uid, room_num, send_time, content, type, ip, status, created_at)
                VALUES (?,?,?,?,?,?,?,?,?)`,
...
func (m *ChatRoomMessageModel) MaxID(ctx context.Context) (int64, error) {
        ...
        err := m.db.QueryRowContext(ctx, `SELECT MAX(chat_room_message_id) FROM `+Tbl("chat_room_message")+``).Scan(&maxID)
        if err != nil {

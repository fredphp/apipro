...
func (l *LiveListLogic) LiveList(in *apipro.PageReq) (*apipro.LiveListResp, error) {
        ...
        allRooms := toLiveRooms(all)
        // zbyy-compatible groups: res[0]=all, res[1]=football, res[2]=basketball, res.hot=hot
        var football, basketball, hot []*apipro.LiveRoom
        hot = make([]*apipro.LiveRoom, 0, len(allRooms))
        for _, r := range allRooms {
                switch r.LiveType {
                case "football", "\u8db3\u7403":
                        football = append(football, r)
                case "basketball", "\u7bee\u7403":
                        basketball = append(basketball, r)
                }
        }
        hot = append(hot, allRooms...)
        return &apipro.LiveListResp{
                List: allRooms[start:end], Total: total,
                All: allRooms, Football: football, Basketball: basketball, Hot: hot,
        }, nil
}
...
func toLiveRooms(rs []fixture.LiveRoom) []*apipro.LiveRoom {
        ...
        for _, r := range rs {
                out = append(out, &apipro.LiveRoom{
                        ...
                        ViewCount: r.ViewCount, CutOutCustomCoverUrl: r.CutOutCustomCoverUrl,
                        MarkType: r.MarkType, LiveStatus: r.LiveStatus,
                })
        }
        return out
}
